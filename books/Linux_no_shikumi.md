...
### 第6章 記憶階層
記憶装置の階層構造
- サイズ小，価格高，アクセス速度速 <--> サイズ大，価格安，アクセス速度遅
- レジスタ，キャッシュメモリ，メモリ，ストレージデバイス

#### キャッシュメモリ
コンピュータ動作の流れ(メモリからの命令読み出しは省略)
1. 命令をもとに，メモリからレジスタにデータを読みだす
2. レジスタ上のデータをもとに計算する
3. 計算結果をメモリ書き戻す

レジスタ上の計算所要時間 >> メモリアクセスの所要時間・レイテンシ

-> 2がどれだけ早くても1，3がボトルネックになる: メモリアクセスのレイテンシに律速(ボトルネックの要因になる事)

レジスタ上の計算とメモリアクセスの所要時間の差を埋めるための**キャッシュメモリ**．1と3を高速化．
キャッシュメモリは一般的にCPUに内臓(内臓されていない場合もある)

キャッシュメモリの処理はハードウェア内で完結

キャッシュサイズ(読みだしサイズはCPUごとに決まってる)の分だけメモリから読みだして，レジスタに読みだす．

ダーティ: データが変更されたキャッシュラインのこと(印)．
ダーティーなメモリラインは後で所定のタイミングでメモリに書き戻される．(ダーティでなくなる)
(ライトバック方式，キャッシュラインがダーティになった瞬間に書き戻すライトスルー方式もある)

#### キャッシュメモリがいっぱいになったら
キャッシュにデータを書き込みしようとするときに，既存のキャッシュラインのうち1つを廃棄する．
廃棄するキャッシュラインがダーティならば，書き戻したうえで廃棄．

全てのキャッシュラインがダーティな時，メモリアクセスのたびにキャッシュライン内のデータが激しく入れ替わるスラッシングが発生．

#### 階層型キャッシュメモリ
x86_64アーキテクチャでは，キャッシュメモリは階層型構造になっている．
- L1 > L2 > L3 (上の方がレジスタに近く，容量が少なく高速に)

キャッシュメモリの情報は'/sys/devices/system/cpu/cpu{#}/cache/index{#}/'といったディレクトリにあるファイルの中身で見られる．
- type: キャッシュするデータの種類．'Data'ならデータのみ，'Code'ならコードのみ，'Unified'ならどちらも．
...
#### キャッシュの実験
cache.c
1. コマンドの第1引数で定められた所定用のメモリを獲得する
2. 獲得したメモリ領域内に，所定の回数，シーケンシャルアクセスをする
3. 1アクセスあたりの所要時間`[2の所要時間]/[2のアクセス回数]`を表示

`-O3`オプションによるコンパイルで最適化し，キャッシュメモリの影響を見えやすく

アクセス時間はおおよそ各キャッシュサイズを境界として階段状に変化する．

#### 参照の局所性
参照の局所性
- 時間的局所性: ある時点でアクセスされたデータは，近い将来に再びアクセスされる可能性が高い．(e.g. ループ処理)
- 空間的局所性: ある時点であるデータにアクセスされると，それに近い場所のデータにアクセスする可能性が高い(e.g. 配列要素の全走査)

これにより，実際のシステムでもキャッシュが有効になる．

#### まとめ
プログラムのワークロードをキャッシュメモリのサイズに収めることで，性能を大きく向上できる．
- データ配置やアルゴリズム，設定を工夫する

#### Translation Lookaside Buffer
プロセスが所定の仮想アドレスのデータにアクセスするための手順
1. 物理メモリ上のページテーブルの参照によって仮想アドレスを物理アドレスに変換
2. 上で求めた物理メモリへのアクセス

2.はキャッシュメモリで高速化可能．1.はできない．

この問題に対して，CPUはキャッシュメモリと同様に高速な'Translation Lookaside Buffer'(TLB)という領域を持つ．

#### ページキャッシュ
CPUからのメモリへのアクセス速度 >> ストレージへのアクセス速度

カーネルのページキャッシュにより速度差を埋める．(キャッシュメモリと似てる)

ページキャッシュではページ単位でデータを扱う．
プロセスがファイルを読みだす際，いったんカーネルのメモリ上にデータとその範囲などの情報をコピーしてから，プロセスのメモリにコピーする．

ページキャッシュは全プロセスに共有される．他は大体キャッシュメモリと同じ．

#### 同期書き込み
ページキャッシュ上にダーティなページが存在する時に，システムが強制電源断が起こると，そのデータは消える．

これが許容できない場合，`open()`システムコールでファイルを開く際に'O_SYNC'フラグを設定する．
これにより，そのあとファイルに`write()`システムコールを発行するごとにページキャッシュとストレージデバイスの両方に書き込まれる

#### バッファキャッシュ
ファイルシステムを使わずに，デバイスファイル(後述)を用いてストレージデバイスに直接アクセスする際などに使う領域(詳しくは第7章)

ページキャッシュと同じように，ストレージ内のデータをメモリ上に置いておく仕組み(ざっくり)

#### ファイル読みだしの実験
同じファイルを2回読みだして，所要時間を比較することでページキャッシュの効果を確かめる．

`$ dd if=/dev/zero of=testfile oflag=direct bs=1M count=1K`: 'testfile'という1Gバイトのファイルを作成．
'oflag=direct'によってダイレクトI/Oによるファイル書き込みでページキャッシュが使用されない．

'testfile'の読みだしと計測
- `$ time cat testfile > /dev/null`
- `free`コマンドの'buff/cache'や`sar -r`コマンドの'kbcached'フィールドでページキャッシュの総量を確認

read-twice.sh で以下を確認
- ストレージデバイスからページキャッシュにデータを読み込むページインの回数
- ページキャッシュからストレージにページを書き出すページアウトの回数
- ストレージデバイスに対するI/O量

- `$ sar -B 1`を別ターミナルで実行
    - ページアウトとページイン，2度目の読み込みでページインが発生しないことを確認
- `$ sar -d -p 1`: ストレージデバイスに対するI/O量も確認
    - `$ mount | grep "on / "`: ルートファイルシステムが存在するストレージデバイス名の確認
    - 'rd_sec/s'と'wr_sec/s'

#### ファイル書き込みの実験
ページキャッシュを使わないダイレクトI/Oによるファイル書き込みの所要時間
``` sh
$ rm -f testfile
$ time dd if=/dev/zero of=testfile oflag=direct bs=1M count=1K
```

ページキャッシュを使う通常のファイル書き込みの所要時間
``` sh
$ rm -f testfile
...
write.sh による統計情報の確認(ファイル読み込みと同じ手順)

#### チューニングパラメータ
ページキャッシュを制御するためのチューニングパラメータの紹介(sysctl)

- 'vm.dirty_writeback_centisecs': ダーティページのライトバック周期
- 'vm.dirty_background_ratio': 全物理メモリのうちダーティページの占める割合がこれを超えるとバックグラウンド処理でライトバック処理の実行
- 'vm.dirty_bytes': 上をバイト単位で指定できる
- 'vm.dirty_ratio': この割合を超えると同期的にライトバックするようになる
- 'vm.dirty_bytes': バイト単位で

これらのパラメータを調節して，メモリが不足したときに突然ダーティページのライトバックが多発しないようにする．

'/proc/sys/vm/drop_caches'に'3'を書き込むと，システムのページキャッシュを(ほぼ)削除することができる．
システム性能へのページキャッシュの影響を確認するのには便利(実運用では使わない)

#### まとめ
sysctlのパラメータ調節

`sar -B`や`sar -d -p`による統計情報

#### ハイパースレッド
CPU使用時間の多くはメモリorキャッシュメモリからのデータ転送待ち時間に空費されている(他にも空費される要因はある．
ハイパースレッド機能によって空費される時間を有効活用できる．

ハイパースレッド機能: CPUコアの中のレジスタなどの一部の資源を複数(一般的には2つ)用意して，それぞれシステムからは論理CPUとして認識されるハイパースレッドに分割するハードウェア機構．
つまり，1つのCPUコアを複数(2つ)の論理CPUに見せる技術?
- スループット性能はそこまで上がらない(20-30%増加で御の字)，プロセスの挙動によっては劣化もある．

#### ハイパースレッド機能が有効な場合
8個の論理CPU(CPUコア数)が，16個の論理CPUとして認識されている．

- `$ time make -j8 >/dev/null 2>&1`: ビルドの並列度8でのビルド
...
### 第7章 ファイルシステム
Linuxではストレージデバイス内のデータにアクセスするのに，ファイルシステムを介してアクセスしている
(直接ストレージデバイスにアクセスしていない)．

どこにどのようなデータがあり，どこが空き領域化を管理する．

単純なファイルシステムの仕様
- 0Gバイト地点からファイルのリストを記録
- 1つのファイルについて，名前，場所，サイズという3つの情報を記録

#### Linuxのファイルシステム
ディレクトリ: ファイルを格納する特殊なファイル

様々なファイルシステムがある("ext4", "XFS", "Btrfs", etc)．
どのファイルシステムであってもユーザからは統一されたインタフェース(システムコール)によってアクセスできる
- `creat()`, `unlink()`: ファイルの作成・削除
- `open()`, `close()`: ファイルを開く・閉じる
- `read()`: 開いたファイルからデータを読みだす
- `write()`: 開いたファイルにデータを書き込む
- `lseek()`: 開いたファイルの所定の位置に移動
- `ioctl()`: 上記以外のファイルシステム依存の特殊な処理

ファイルのデータ読み出し手順
1. カーネル内の全ファイルシステム共通処理によって操作対象のファイルのファイルシステムを判別する
2. 各ファイルシステムを扱う処理を呼び出して，システムコールに対応した処理をする
3. データの読み書きをする際は，デバイスドライバに処理を依頼
4. デバイスドライバがデータを読み書き

#### データとメタデータ
ファイルシステムの2つのデータの種類
- データ: ユーザが作成した文書や画像，動画，プログラムなどの内容
- メタデータ: ファイルの名前やストレージデバイス上の位置，サイズなどの補助的な情報
    - 種類: データを保持する通常のファイルか，ディレクトリか，その他か種類を判別する情報
    - 時刻情報: 作成した時間，最後にアクセスした時間，最後に内容を変更した時間
    - 権限情報: どのユーザがファイルにアクセスできるか

#### 容量制限
クォータ(quota): 用途ごとに使用できるファイルシステムの容量を制限する機能
- ユーザクォータ: ファイルの所有者となるユーザごとに容量を制限
- ディレクトリクォータ(プロジェクトクォータ): 特定のディレクトリごとに容量を制限
- サブボリュームクォータ: ファイルシステム内のサブボリュームという単位ごとに容量を制限．大体ディレクトリクォータと使いかたは同じ．

#### ファイルシステムの不整合
ファイルシステムの内容に不整合(e.g. ファイルシステムのデータをストレージに読み書きしている最中にシステムの電源が落ちるときなど)

アトミックな処理: 不可分な処理

不整合が発生すると，ファイルシステムが(遅かれ早かれ)検出する
- ファイルシステムがマウントできなくなる
- 読み出し専用モードで再マウント(remount)
- システムのパニック(最悪)

ファイルシステム不整合を防ぐ2つの広く使われている技術
- ジャーナリング(ext4, XFS)
- コピーオンライト(Btrfs)

#### ジャーナリング
ジャーナル領域: ユーザに認識できないメタデータ．
ファイルシステムを更新する際に以下のような手順を踏む．
1. 更新に必要なアトミックな処理の一覧を，いったんジャーナル領域に書き出す(ジャーナルログを書き出す)
2. ジャーナル領域の内容に基づいて実際にファイルシステムの内容を更新する

ジャーナルログ更新中の強制電源断 -> ジャーナルログを廃棄

実データの更新中の強制電源断 -> ジャーナルログを最初から再生

#### コピーオンライト
ファイルシステムのデータ格納方法について(前提，ext4やXFSなどの従来型)．
- いったんファイルを作成したらそのファイルの配置場所は原則変わらない．
内容更新は同じ場所に書き込む．

Btrfsなどのコピーライト型のファイルシステム
- 更新するごとに別の場所にデータを書き込む．
- 更新されるデータを全て別の場所に書き込んでから，リンクを張り替える．

更新途中に強制電源断が発生しても，作りかけのデータを削除すれば不整合は解消される．

#### それでも防げない場合
ファイルシステムのバグに起因する不整合は少数ながら存在する．

#### ファイルシステム不整合への対策
一般的にはファイルシステムの定期的なバックアップによる復元．

バックアップをしていない場合

ファイルシステムに用意されている復旧用コマンドの利用(非推奨)
- `fsck`: どのファイルシステムにも共通して存在するコマンド．
    - 整合性確認と修復のためにファイルシステムを全走査するため，所要時間がかかる．
    - 結局失敗になることが多い
    - ユーザが望む状態に復元するとは限らない．無理矢理マウントできるようにするコマンド．不整合が発生しているデータやメタデータを容赦なく削除する．

#### ファイルの種類
通常のファイルと，ディレクトリに加えて，デバイスファイルがある．

動作しているハードウェア上のデバイスをほぼすべてファイルとして表現する(ネットワークアダプタは例外)

`open()`や`read()`によってアクセス可能．デバイス固有の複雑な操作は`ioctl()`システムコールを使う．

Linuxがファイルとしてアクセスするデバイスの分類
- キャラクタデバイス
- ブロックデバイス

'/dev'以下に各デバイスファイルが存在する．
デバイスファイルのメタデータに保存されている以下の情報によって個々のデバイスを一意に識別
- ファイルの種類(キャラクタデバイス or ブロックデバイス)
- デバイスのメジャー番号
- デバイスのマイナー番号

`ls`コマンドで，先頭の文字が"c"であればキャラクタデバイス，"b"であればブロックデバイス．
第5フィールドがメジャー番号，第6フィールドがマイナー番号

#### キャラクタデバイス
読み出しと書き込みはできるが，シークはできない．

代表的なキャラクタデバイス
- 端末: bashなど，シェルを介してコマンドを実行するための画面，あるいはウィンドウ．
- キーボード
- マウス
...
`$ ps ax | grep bash`: bashに結びつけられている端末に対応するデバイス名の確認．書籍では"/dev/pts/9"
...
例えば2つ端末を立ち上げて，片方の端末で2つ目の端末にechoすれば，2つ目の端末を操作せずに"hello"が表示される．

アプリケーションが端末のデバイスファイルを直接操作することは少ない．
Linuxが提供するシェルやライブラリが直接操作する．

bash上の操作は最終的にはデバイスファイルの操作に変換されることを理解する．

#### ブロックデバイス
ブロックデバイスにはファイルの読み書きに加え，ランダムアクセスができる．
代表的なブロックデバイスはHDDやSSDなどのストレージデバイス．
ブロックデバイスへのアクセスで，通常のファイルのようにストレージの所定の位置にある所定のデータにアクセスできる．

ブロックデバイスにはそこにファイルシステムをマウントしてそれ経由で使う．

ブロックデバイスを直接操作するのは以下のような場合
- パーティションテーブルの更新(`parted`コマンドなど)
- ブロックデバイスレベルのデータのバックアップとリストア(`dd`コマンドなど)
- ファイルシステムの作成(`mkfs`コマンドなど)
- ファイルシステムのマウント(`mount`コマンドなど)
- `fsck`

ブロックデバイスを直接操作する．
- `$ mkfs.ext4 /dev/sdc7`: 適当なパーティション上にext4ファイルシステムを作る
- 作成したファイルシステムをマウントして適当な名前で適当な内容のファイルを作る
```sh
# mount /dev/sdc7 /mnt/
...
# umount /mnt
```
- `$ strings -t x /dev/sdc7`: ファイルシステムのデータから，文字データを1行に1つずつ，第1フィールドにファイルオフセット，第2フィールドに見つかった文字列の形式で表示できる．以下の2点を確認
    - "lost+found"ディレクトリ，および”testfile"というファイル名(メタデータ)
    - ファイルの内容である"hello world"という文字列(データ)

testfileの中身をブロックデバイスから変更する
```sh
...
注意
- 他バージョンや別のファイルシステムでうまくいくとは限らない
- 危険なので，テスト用のファイルシステムで実施するべきである

#### さまざまなファイルシステム
Linuxで使用されるext4, XFS, Btrfs以外のファイルシステム

#### メモリベースのファイルシステム
メモリ上にファイルシステムを作成する"tmpfs"．電源を切るとデータがなくなってしまうが，ストレージデバイスへのアクセスが一切ないため高速にアクセス可能．

再起動後に残っている必要のない'/tmp'や'/var/run'に使用されることが多い

#### ネットワークファイルシステム
ネットワークを介して繋がっているリモートホスト上のファイルにアクセスする．
- Windowsホスト上にアクセスするなら"cifs"
- Linuxなら"nfs"

#### 仮想ファイルシステム
カーネル内の情報を得たり，カーネルの挙動を変更するためのファイルシステム
...
- /proc/{pid}/stat: プロセスの状態，これまでに使用したCPU時間，優先度，仕様メモリ量など
- /proc/cpuinfo: CPUに関する情報
- /proc/diskstat: ストレージデバイスに関する情報
- /proc/meminfo: メモリに関する情報
- /proc/sys以下: カーネルの各種チューニングパラメータ．sysctlコマンドと/etc/sysctl.confによって変更するパラメータと1対1対応している

##### sysfs
カーネルのプロセスに関するもの以外の雑多な情報がprocfsに置かれるようになったので，
それらを配置する場所として作られた．

'/sys'以下にマウントされる
- /sys/devices以下: デバイスに関する情報
- /sys/fs以下: 各種ファイルシステムに関する情報

##### cgroupfs
cgroup: 1つのプロセスあるいは複数個のプロセスから成るグループに対して，さまざまなリソース使用量の制限をかける機能．cgroupfsというファイルシステムを介して操作(rootのみ)．

/sys/fs/cgroup以下にマウントされる

cgroupに制限がかけられるリソース
- CPU: グループがCPUの全リソースのうちの50%など．/sys/fs/cgroup/cpu以下のファイルの読み書きによって制御
- メモリ: グループが物理メモリのうちの1Gバイトなど．/sys/fs/cgroup/memory以下のファイルによって制御

たとえばdockerなどのコンテナ管理ソフトやvirt-managerなどの仮想マシン管理ソフトによって個々のコンテナや仮想マシンのリソースを制限するために使用される．

#### Btrfs
ext4やXFSはUNIXが作られたころからある，単純な機能のみを提供．
Btrfsのような豊富な機能を提供するファイルシステムの登場．

##### マルチボリューム
ext4やXFSは，1つのパーティションに対して1つのファイルシステムを作成する．

Btrfsは，1つ以上のストレージデバイス/パーティションから大きなストレージプールを作成し，その上にマウント可能なサブボリュームという領域を作成する．
- ストレージテーブルはLVM(logical volume manager; 論理ボリューム管理)におけるボリュームグループ
- サブボリュームはLVMにおける論理ボリュームとファイルシステムを足したものに近い

Btrfsは，従来のファイルシステムにLVMの機能を備えたものと考えられる

ファイルシステムのリサイズ処理が不要で(運用を止めることなく)ストレージデバイスの追加・削除・交換が可能．

##### スナップショット
サブボリューム単位でスナップショットを採取
- データを参照するメタデータの作成，およびスナップショット内のダーティページのライドバック
- ファイルをコピーする際に高速かつ空間的コストがかからない

#### RAID
ファイルシステムレベルでRAID構成を組むことができる
- サポートしているRAID: 0, 1, 10, 5, 6, dup(シングルデバイス用の同じデータを同じストレージデバイスに二重化)
- ファイルシステム全体でRAIDのレベルを決定

#### データ破損検知/修復
データ破損の検知とその修復(RAIDに依存)

Btrfsはデータ，メタデータとともに所定のデータ長ごとにチェックサムを持つことで，破損検知する．
チェックサムエラーを読み取り時に検出すると，そのデータは捨てて依頼先にI/Oエラーを返す．

RAIDでの修復が可能な場合は，読み出し元はデータ破損を意識させずに処理を継続できる．

ext4やXFSではメタデータの破損の検出・破棄のみ．メタデータ&データの破損検出・破棄・修復ができるのは(書籍執筆時は)Btrfsのみ．

### 第8章 ストレージデバイス
HDDについて．I/O性能を向上させるカーネル機構について．ブロックデバイス層について．SSHについて．

#### HDDのデータ読み書きの仕組み
データを磁気情報で表現しプラッタ(磁気ディスク)にセクタ単位で記録．
セクタは半径方向および円周方向に分割され通番が振られる．

HDDからのデータ転送の流れ
1. デバイスドライバがデータの読み書きに必要な情報(セクタ番号，セクタの数，アクセスの種類)をHDDに渡す
2. スイングアームとプラッタを動かして，アクセス先のセクタ上に磁気ヘッドの位置を合わせる
3. データを読み書きする

#### HDDの性能特性
- 回転だけでデータを一気に読める
    - 一度に読める量はHDDごとに制限
- 読み取るデータがセクタ上で離れていると，アクセス時間が伸びる
    - ファイル内のデータを連続する，あるいは近い領域に配置する
    - 連続する領域へのアクセスは，複数回に分けずにひとまとめにする
    - ファイルには，なるべく大きなサイズでシーケンシャルアクセスをする

#### HDDの実験
ブロックデバイスのデータを直接読み書きする．必ず未使用のパーティションを用意してそれをテストに用いる．
...
io.cによる実験
- open()関数に"O_DIRECT"フラグを与えることでダイレクトI/Oを使う．(カーネルによるI/O支援機能を使わないように)
- ioctl()関数によって指定したデバイスに対応するストレージデバイスのセクタサイズを得る
- バッファ用メモリ領域の確保にはmalloc()関数ではなくposix_memalign()関数を使う．獲得するメモリのアドレスを指定した数の倍数にする．ストレージデバイスのセクタサイズの倍数である必要があるので．
- fdatasync()関数によって，発行しているI/Oの完了を待つ．ダイレクトI/Oではない通常のI/Oの場合，write()関数はI/Oの発行依頼をするだけで完了を待たないから

#### シーケンシャルアクセス(アクセスパターン)
一回あたりのI/Oサイズが大きくなるほどスループット性能が向上するが，サイズが1Mバイトになったころから頭打ちに(このHDDが一度にアクセスできるデータ量の上限)．
上限を超えると，ブロックデバイス層において，アクセスは複数に分割される．

#### ランダムアクセス(アクセスパターン)
シーケンシャルアクセスの性能よりも全体的に悪い．
特にI/Oサイズが小さいとき，アクセス待ち時間が多くなる．

#### ブロックデバイス層
ブロックデバイスの処理は共通した処理が多いため，各デバイスのデバイスドライバ内ではなくカーネル内のブロックデバイス層が担当する．

#### I/Oスケジューラ
ブロックデバイスへのアクセス要求を一定期間ためておいて，加工してからデバイスドライバに要求することでI/O性能の向上を目指す
- マージ: 複数の連続するセクタへのI/O要求を1つにまとめる
- ソート: 複数の不連続なセクタへのI/O要求をセクタ番号順に並べ替える

#### 先読み
空間的局所性による先読み(read-ahead)機能

アクセス後に，その直後とそれに続く領域を事前に読み込んでおく．
推測が正しくなければ単に読み込んだデータは破棄．

#### 実験
I/O支援機能を使った実験

##### シーケンシャルアクセス
スループット性能がI/Oサイズが小さい時点から限界に近くなっている．
読み出しについて，これが先読みの効果．

- 別端末で，`iostat -x`: 読み出しの時間が短くなっている．
    - "rrqm/s" 読み出し処理におけるマージ数
        - 実験ではI/Oスケジューラが働かないので0回になる
        - 複数のプロセスを並列に読みだす場合などにI/Oスケジューラは働く
    - 書き込みの高速化はマージ処理のおかげ"wrqm/s"

##### ランダムアクセス
I/Oサイズが大きくなるほどスループット性能がシーケンシャルアクセスに近づくが，同程度にはならない．

ランダムなので先読みは破棄される．

書き込みの際は，サイズが小さい時にI/Oスケジューラの効果が見られる．
たまたまランダムなアクセス領域が隣り合っていてマージが発生することもある．

#### SSDのしくみ
HDDにおける物理的な動作が，電気的(データ検索)に．

SSDを用いた実験．
ランダムアクセスにおいて，特にSSDがHDDより優れている．
ランダムアクセスとシーケンシャルアクセスの差もHDDほど開いていない．

先読みも行う．

書き込みについて，I/Oサイズが大きいときはI/O支援機能がない場合よりもある場合のほうがスループット性能が低く．
複数のI/O要求をためておく処理のオーバーヘッドがSSDにおいては無視できなくなる．アクセスが早いため．
ソートの効果があまり期待できないため．

#### まとめ
ユーザが意識しなくてもカーネルの支援機能によってある程度のアクセス最適化がされている．
が，すべての場合において性能が向上するわけではない．

データの配置に気を付ける．

### あとがき
- コンピュータの構成と設計 上・下
    - ハードウエアのアーキテクチャに関する古典的名著
- What Every Programmer Should Know About Memory
    - メモリについて実験をもって確かめる
- ふつうのLinuxプログラミング 第2版
    - C言語を用いたLinuxにおけるプログラミングについて
- 詳細システム・パフォーマンス
    - LinuxとSolarisの性能測定について
