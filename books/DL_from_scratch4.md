...
- オフライン強化学習 (offline reinforcement learning)
    - 過去に集められた経験データ(オフラインデータ)をうまく活用するアプローチ
    - オフラインデータのみを使い，環境とのやり取りを行わず最適な方策を推定する
    - 方策オフ型の手法に関連
    - 方策の評価，改善に課題(重点サンプリングなどによる解決)
- 模倣学習 (imitation learning)
    - 人間の熟練者(エキスパート)から模倣
    - エキスパートの経験データを経験再生としてDQNに追加する(Atari)

MDPとして定式化する際のヒント
- 強化学習の多くの理論はMDPを前提としている
- 環境における状態，行動，報酬の3情報が柔軟である(どのようにも設定できる)
- エージェントの行動のレベルも高いものから低いものまで考えられる
- 時間の単位も任意

MDPで設定が必要な時効
- 取り組む問題はエピソードタスクか，それとも連続タスクか
- 報酬の値は?(報酬関数の設定)
- エージェントの取りえる行動は?
- 何を環境の状態とするか?
- 収益の割引率 $\gamma$ の値は?
- どこまでが環境で，どこまでをエージェントとするか?

良い定式化を行うためのヒント
- 科学より芸術に近い側面がある(理論より経験や直感が役に立つ)
- 強化学習の分野で有名な研究を見る(特に，どのようにMDPとして定式化しているか)

汎用人工知能システム (artificial general intelligence; AGI)
- 強化学習はAGIの実現においてキーになる
- Reward is Enough 「知性やそれに関連する能力は，報酬の総和の最大化で十分理解できる」
